...
class AnrotFrame_Exception(Exception):
    def __init__(self,err='HI221GW Frame Error'):
        Exception.__init__(self,err)

class AnrotFrame_NoValid_Exception(AnrotFrame_Exception):
    def __init__(self,err='No valid frame received'):
        Exception.__init__(self,err)

class AnrotFrame_NotCompleted_Exception(AnrotFrame_Exception):
    def __init__(self,err='No full frame received'):
        Exception.__init__(self,err)

class AnrotFrame_ErrorFrame_Exception(AnrotFrame_Exception):
...
def sample_rate_timer_cb(sample_timer):
    ...
    print("Rate：",SampleRate,datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))
...
def find_frameheader(buffer_list:list):
    ...
    while True:
        ...
        try:
            ...
            raise AnrotFrame_NotCompleted_Exception
...
def _verify_frame_length(buffer_list:list, header_pos):
    ...
    # 判斷幀長度是否合法
    if frame_len >= 1024:
        raise AnrotFrame_ErrorFrame_Exception
